...
outfileName = './data/soundcloud_footwork_users.csv'
...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    datefmt='%m-%d %H:%M',
                    filename='./logs/footwork_users.log',
                    filemode='w')
...
try:
	# find all users with footwork in description, tag, etc.
	...
	if('collection' in response_dict):
		...
		logging.info('Got ' + str(total_entries) + ' results from first request ')
	while (next_href):
		counter = counter + 1
		...
		if('collection' in response_dict):
			...
			data.to_csv(outfileName, mode='a', header=False)
			total_entries = total_entries + len(data)
			logging.info('Got ' + str(len(data)) + ' results from request ' + str(counter)
				+ '; Total: ' + str(total_entries))
		else:
		...
		sleep(SLEEP_TIME)
	logging.info('All users pulled. Finishing at ' + str(datetime.now()))
